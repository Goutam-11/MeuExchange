...
type DashboardPayload = {
  ...
  assets: Array<{ id: string; name: string; atsTemplate: string; use: string; requiredTerms: readonly string[] }>;
  state: { repos: unknown[]; orders: unknown[]; trades: unknown[]; distributions: unknown[] };
  intents: Array<{ id: string; kind: string; status: string; createdAt: string }>;
};
...
function render(payload: DashboardPayload) {
  ...
  setText('#api-mode', payload.mode === 'demo' ? 'Demo mode' : 'Testnet mode');
  ...
  assets.innerHTML = payload.assets.map((asset) => `<article class="surface asset-card"><span class="eyebrow">${escapeHtml(asset.atsTemplate)} TEMPLATE</span><h3>${escapeHtml(asset.name)}</h3><p>${escapeHtml(asset.use)}</p><footer><span>${asset.requiredTerms.length} required terms</span><span>View terms ↗</span></footer></article>`).join('');
  const activity = root.querySelector('#activity-list')!;
  const events = [...payload.state.trades.map(() => 'ATS transfer settlement'), ...payload.state.repos.map(() => 'Repo collateral workflow'), ...payload.intents.map((intent) => `${intent.kind} intent prepared`)];
  activity.innerHTML = events.length ? events.slice(-5).reverse().map((event, index) => `<div class="activity-row"><span>${escapeHtml(event)}</span><span>${index === 0 ? 'Just now' : 'Recorded'}</span></div>`).join('') : '<div class="loading-row">No activity yet. Prepare an ATS intent to begin.</div>';
  const table = root.querySelector('#intent-table')!;
  root.querySelector('#queue-count')!.textContent = `${payload.intents.length} request${payload.intents.length === 1 ? '' : 's'}`;
  table.innerHTML = payload.intents.length ? payload.intents.slice().reverse().map((intent) => `<tr><td>${escapeHtml(intent.id.slice(0, 18))}…</td><td>${escapeHtml(intent.kind)}</td><td class="intent-status">${escapeHtml(intent.status.replace('_', ' '))}</td><td>${new Date(intent.createdAt).toLocaleString()}</td></tr>`).join('') : '<tr><td colspan="4" class="loading-row">No intents awaiting signature.</td></tr>';
}
...
root.querySelector('#prepare-issuance')!.addEventListener('click', async () => {
  ...
  try {
    const response = await fetch('/api/intents/issuance', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ assetType: 'fixed-income-note', name: (root.querySelector('#issuance-name') as HTMLInputElement).value, symbol: (root.querySelector('#issuance-symbol') as HTMLInputElement).value, isin: 'MEU-DEMO-0001', currency: 'USD', units: '1000', configId: 'dashboard-demo-config', configVersion: 1, ownerAccount: walletAccount || undefined }) });
    const result = await response.json() as { error?: string };
    if (!response.ok) throw new Error(result.error || 'The API rejected the issuance intent');
    showMessage('ATS issuance intent prepared. Connect a wallet to sign it; no transaction was broadcast.');
    await refresh();
    activateTab('intents');
  } catch (error) { showMessage(error instanceof Error ? error.message : 'Could not prepare the issuance intent'); }
